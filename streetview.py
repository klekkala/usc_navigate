...
# to get all the possible waypoints of all the streets in USC.
# 2. Query the Directions API using the waypoints in order to get all the possible coordinates along the aroutes.
...
class FetchImageThread(threading.Thread):
    ...
    def updateURL(self):
        self.previewURL = "https://maps.googleapis.com/maps/api/streetview?size=%dx%d&location=%10.6f,%10.6f&heading=%6.2f8&pitch=%6.2f&fov=%6.2f&key=%s" % (
            self.w, self.h, self.lat, self.lon, self.heading, self.pitch, self.fov, apikey)

        self.metaURL = "https://maps.googleapis.com/maps/api/streetview/metadata?size=%dx%d&location=%10.6f,%10.6f&heading=%6.2f8&pitch=%6.2f&fov=%6.2f&key=%s" % (
            self.w, self.h, self.lat, self.lon, self.heading, self.pitch, self.fov, apikey)

        return self.previewURL, self.metaURL
    ...
    def pitch2hz(self, pitch):
        ...
        coff = [0.0003148498860, -0.0016488159544,
                2.7050819016882, 148.2091043844485]
    ...
    def heading2vt(self, heading_offset):
        ...
        coff = [-0.0003556245191,  0.0005850890405, -
                2.6116922329983,  152.9177984106553]
    ...
    def getAngleBetweenCoord2(self, p1, p2):
        ...
        bearing = (math.degrees(math.atan2(dLong, dPhi)) +
                   360.0) % 360.0 - 180.0
        #print bearing
        return bearing

    # Get all the steps from the queried json file into a list.
    def getSteps(self, jsonfile):
    ...
    def getRoute(self):
        ...
        try:
            if r.status_code == 200:
                ...
                # Save the file:
                with open("%sdata.json" % (dirname), "w") as f:
                    json.dump(jsonobj, f)

                steps = self.getSteps(jsonobj)
    ...
    # Return the latitude, longitude and the id of the image.
    def getMetadata(self):

        r = None
        r = sendRequest(self.metaURL)
    ...
    def run(self):
        ...
        # Try open the file. If the file does not exist then use getRoute method:
        try:
            with open("%sdata.json" % (dirname), "r") as f:
            ...
            print("File %sdata.json does not exist. Querying the Google API for the data." % (dirname))
            steps = self.getRoute()

        if steps is None:
            print("Response from API is None. Check the request.")
            return
        ...
        for s in steps:
            ...
            pts = gistfile1.decode(polyline)
            num_pts = len(pts)
            # What is turning image??? Why skip the first 12 and the last 5??
                        ...
                        for i in range(0, self.max_index):
                            ...
                            if self.options.preview:
                                if index % (SAMPLING_WAYPOINTS*10) == 0:
                                    ...
                                    # left green
                                    cv2.line(img, (0, hzline),
                                                (self.w, hzline), green, 1)
                                    cv2.line(img, (vtline, 0),
                                                (vtline, self.h), blue, 1)
                                    cv2.imshow("image", img)
                                    cv2.waitKey(0)

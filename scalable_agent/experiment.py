...
sys.path.append('src')
sys.path.append('scalable_agent')

from beogym import BeoGymEnv
...
def build_actor(agent, env, level_name, action_set):
  ...
  dummy_agent_output, _ = agent( # This syntax is weird. Did the _build method got called?
      (initial_action,
...
def create_beogym(seed):

  p = py_process.PyProcess(BeoGymEnv, seed)
  return environments.FlowEnvironment(p.proxy)

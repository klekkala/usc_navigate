...
class _TFProxy(object):
  ...
  def __getattr__(self, name):
    def call(*args):
      ...
      def py_call(*args):
        try:
          self._out.send(args) # This is sent to the _in connection. Look at _worker_fn.
          result = self._out.recv() # The result from _in connection in _worker_fn is received here.
          ...
          if isinstance(e, IOError):
            ...
            raise
      # As an op node of the TF graph, the method name and it args are sent to the worker process that has the _in connection to get the results.  
      result = tf.py_func(py_call, (name,) + tuple(args), flat_dtypes,
                          name=name)
      # Not sure whats going on down here with the result.
  ...
  def _start(self):
    ...
    self._process = multiprocessing.Process(
        ...
        args=(self._type, self._constructor_kwargs, in_)) # The worker (actor) is the _process that has the _in connection.
    self._process.start()
  ...
  def _worker_fn(self, type_, constructor_kwargs, in_):
    try:
      ...
      while True:
        # Receive request.
        serialized = in_.recv()
        # None is sent from _out when _close is called.
        ...
        # Compute result. The _in process runs the method in DeepmindLab environment.
        results = getattr(o, method_name)(*inputs)
        if results is not None:
          results = nest.flatten(results)

        # Respond. Send the results to the _out.
        in_.send(results)
    except Exception as e:
